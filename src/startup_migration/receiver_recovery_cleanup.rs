...
pub(super) fn up(home: &Path) -> Result<()> {
    for state in super::receiver_model::workspace_states(home) {
        ...
        .with_context(|| {
            format!(
                "upgrade receiver recovery cleanup state {}",
                state.path.display()
            )
        })?;
    }
...
pub(super) fn down(home: &Path) -> Result<()> {
    for state in super::receiver_model::workspace_states(home) {
        crate::state::receiver_recovery_cleanup_schema_down(&state.path).with_context(|| {
            format!(
                "downgrade receiver recovery cleanup state {}",
                state.path.display()
            )
        })?;
    }
