...
pub(super) fn up(home: &Path) -> Result<()> {
    for state in super::receiver_model::workspace_states(home) {
        ...
        .with_context(|| {
            format!(
                "upgrade receiver session registration state {}",
                state.path.display()
            )
        })?;
    }
...
pub(super) fn down(home: &Path) -> Result<()> {
    for state in super::receiver_model::workspace_states(home) {
        crate::state::receiver_registration_schema_down(&state.path).with_context(|| {
            format!(
                "downgrade receiver session registration state {}",
                state.path.display()
            )
        })?;
    }
