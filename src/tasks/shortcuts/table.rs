...
pub(crate) const ALL: &[Shortcut] = &[
    ...
    Shortcut {
        keys: "h j k l",
        label: "move",
        desc: "Collapse / down / up / expand in the tree — the vim aliases for ← ↓ ↑ →, cursor movement, not a command",
        group: Group::BrainDirectory,
        in_footer: false,
        commands: &[],
    },
    Shortcut {
        keys: "H / L",
        label: "sibling",
        desc: "Jump to the first / last row among the selected node's siblings, staying inside its own directory (Home / End take the whole tree) — cursor movement, not a command",
        group: Group::BrainDirectory,
        in_footer: false,
        commands: &[],
    },
    Shortcut {
        keys: ".",
        label: "hidden",
        desc: "Show / hide dotted names in the tree. Persists to portable config, so the choice survives a restart and reaches the workspace's other machines",
        group: Group::BrainDirectory,
        in_footer: false,
        commands: &[Global(GlobalAction::ToggleHiddenFiles)],
    },
    Shortcut {
        keys: "^G",
        label: "pdf",
