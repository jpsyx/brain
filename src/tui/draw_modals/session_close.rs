...
use crate::tui::modal_state::SessionClosePickerState;

pub(crate) fn draw_session_close_picker(
    frame: &mut Frame,
    state: &SessionClosePickerState,
    area: Rect,
) {
    ...
    .split(inner);
    frame.render_widget(
        Paragraph::new(" Select a session to close").style(Style::new().fg(TEXT_DIM)),
        chunks[0],
    );
    frame.render_widget(
    ...
    let lines: Vec<Line<'static>> = state
        ...
        .map(|(index, row)| {
            ...
            let title_style = if row.closeable {
