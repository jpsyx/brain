...
#[test]
fn enter_forwarded_to_an_idle_agent_marks_a_turn_active() {
    assert!(brain_key_starts_turn(KeyCode::Enter));
    assert!(!brain_key_starts_turn(KeyCode::Char('y')));
    assert!(!brain_key_starts_turn(KeyCode::Tab));
}
