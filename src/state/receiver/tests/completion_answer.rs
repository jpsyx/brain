...
#[test]
fn reused_pid_on_another_brain_instance_does_not_preserve_the_old_cleanup_fence() {
    let fixture = super::binding::completion_fixture(ReceiverJobState::Processing);
    fixture
        .db
        .complete_receiver_job_with_binding(&fixture.request())
    ...
    let cleanup = fixture
        .db
        .next_receiver_answer_cleanup()
        .expect("inspect reused-PID cleanup fence")
        .expect("the mismatched incarnation proves the old lock is stale");

    assert_eq!(cleanup.job_id(), fixture.job_id);
}
