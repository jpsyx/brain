...
impl RegistryStore {
    ...
    pub fn save_atomic_to(path: &Path, registry: &MachineRegistry) -> Result<(), RegistryError> {
    ...
    ///
    /// This clones and mutates a candidate, validates and atomically persists
    /// it, and only then replaces `registry`. The mutation methods directly on
    ...
    pub fn update<T>(
        ...
        if let Some(temporary) = &self.temporary_path {
            Self::save_atomic_to_with_temporary(&self.path, &candidate, temporary)?;
        } else {
            Self::save_atomic_to(&self.path, &candidate)?;
        }
        *registry = candidate;
        Ok(result)
    }
