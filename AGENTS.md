...
| Brain-env schema, the `brain env` command, the `markdown-to-pdf` prerequisite, the `sync` block, or **root resolution** (`root` is a brain-env key in `~/.config/brain/env.json`; the legacy `~/.config/brain-root` pointer is read-only back-compat, auto-migrated into `env.json` on first run — root is *not* a `brain config` var) | `docs/config.md` + `docs/data-model.md` (env store + schema + migration in `src/env/`; root resolution in `src/paths.rs`; the parse-only `sync` block in `src/sync/`) |
