...
- **Receiver dispatch state.** Composed `tui::app_brain::tests` drive the one
  ...
  failure, reply delivery, task reload, and sync push. A focused tab test proves
...
- **Opaque-ingress workspace routing.** `server/router.rs` exhaustively proves
  ...
  aliases retain their target, ordered type-parameter defaults, and generic
  substitutions at the definition scope. Explicit use-site type arguments
  override defaults; omitted defaults expand in parameter order and may refer
  to earlier parameters. Lifetime and const arguments cannot shift type
  argument positions. An omitted parameter without a default remains opaque.
  Nearest declarations stop outer alias lookup, alias and default chains
  terminate cycles, and unknown or ambiguous targets retain the fail-closed
  glob fact.
  ...
  name cannot overwrite one another. It
  rejects reachable main-panel operations, activity sampling, Unix socket
  accepts and reads (including reads before job decoding), inbound channels,
  and in-memory job queues. It also counts exactly one production receiver tick
  ...
  orders, controller fields and return chains, Unix sockets, channels, and
  queues. Negative fixtures prove a qualified safe trait call cannot inherit
  its peer's forbidden edge, and unrelated same-name methods and ordinary
...
| `tui::app_brain::tests::{receiver_durable_launch,receiver_durable_lifecycle,receiver_durable_binding_completion,receiver_durable_resume_completion}` | Composed production-tick coverage for disabled/busy gating, freshness-first exact renewal, timestamp/job-ID FIFO, an arrival waiting unclaimed behind an active run, isolated all-frontend controller launch, exact artifact correlation, atomic native-binding plus terminal commit, resumed Codex and OpenCode second-run completion when the durable binding already equals the lifecycle-native session, retained retry after binding mismatch or SQLite error, barrier-driven lifecycle rotation after validation that preserves the old artifact and active run, lost ownership, progressed stale-state refusal, spawn and child-exit retry, reply/task/sync terminal effects, and unchanged view, tab, visibility, and focus at launch, close, and next launch. Tests use injected clocks and event barriers rather than fixed sleeps. |
