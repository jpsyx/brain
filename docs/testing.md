...
| `tui::receiver::planning_tests` + `agent::adapter_tests::contract` | Table-driven Claude/Codex/OpenCode rendering from an already-authorized Fresh/Resume choice: empty transcript, UTF-8-safe newest-context truncation, bounded attachment references, complete shell-argument budgeting, and both fresh/resume command translations with a non-blank initial prompt. |
