...
The BR-18 cutover closes with this restart matrix. Each row is exercised after
database reopen or fresh `App` construction; the cited suites also assert that
later FIFO work either remains correctly fenced or advances without a surviving
process-local object.

| Durable phase | Restart outcome and primary coverage |
| --- | --- |
| `queued` | Reopen retains the immutable job; the next enabled App claims the oldest row. Covered by receiver acceptance/claim tests and `receiver_durable_launch`. |
| `claimed` | Orderly shutdown records one bounded planning retry; expiry reconciliation releases the stale owner before later FIFO work. Covered by `receiver_durable_shutdown` and receiver reconciliation tests. |
| `launching` | Successful-spawn ambiguity remains fenced from replay until exact cleanup or explicit terminalization. Covered by `receiver_durable_slow_launch_effects`, `receiver_recovery_shutdown_fence`, and recovery-state tests. |
| `launched`, `accepted`, `processing` | Fresh App construction never replays the inbound turn; exact recovery policy, lifecycle evidence, and cleanup attribution decide the next step. Covered by `receiver_durable_process_restart`, `receiver_recovery_restart`, and `receiver_recovery_frontend_matrix`. |
| recovery | A fresh App cleans the exact stale native run, then resumes the persisted frontend and native session for Claude, Codex, and OpenCode. Covered by `receiver_recovery_restart`, `receiver_recovery_frontend_matrix`, and owner-loss restart tests. |
| `cleanup-gated` response | Provider claim remains blocked until exact native/artifact cleanup acknowledgement promotes the durable row to `ready`; later work then advances. Covered by `receiver_recovery_effects`, `receiver_durable_answer_handoff`, and cleanup restart tests. |
| `answer-ready` | The agent lane is already released; a fresh delivery worker claims the immutable response independently. Covered by completion-answer tests and `receiver_durable_delivery`. |
| `delivering` | Shutdown cancels only local provider work; an expired durable provider-IO lease reconciles by provider policy before another claim. Covered by `receiver_durable_delivery` and delivery-store restart tests. |
| `retrying` | Due provider retries resume from the frozen envelope, while an expired idempotency window terminalizes explicitly without provider IO. Covered by delivery-store reopen tests and `receiver_durable_delivery`. |
| `acknowledged` | Reopen and repair preserve the exact provider acknowledgement and finish the job once; lost result commits follow provider-specific replay or ambiguity policy. Covered by delivery fallback and `receiver_durable_delivery_post_ack`. |
| `failed`, `done` | Terminal rows are never reclaimed as agent work and cannot block a later queued job. Covered by terminal recovery, invalid-lineage restart, fallback reopen, and delivery acknowledgement tests. |
