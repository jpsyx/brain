...
- **Opaque-ingress workspace routing.** `server/router.rs` exhaustively proves
  ...
  aliases retain their target and generic substitutions at the definition
  scope. Nearest declarations stop outer alias lookup, alias chains terminate
  cycles, and unknown or ambiguous targets retain the fail-closed glob fact.
