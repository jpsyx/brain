...
proves no replay plus advancement of a later queued job. The recovery row runs
this reconstruction independently for Claude, Codex, and OpenCode through
...
| `launched`, `accepted`, `processing` | The fresh App never replays the inbound turn; durable lifecycle evidence selects recovery, and later FIFO work advances after completion. Directly covered by `receiver_durable_reconstruction_matrix`; process-restart suites remain focused support. |
...
| `answer-ready` | A fresh delivery worker claims the immutable response independently and later FIFO work advances. Directly covered by `receiver_durable_reconstruction_matrix`; completion-answer and delivery suites remain focused support. |
| `delivering` | The fresh App reconciles expired durable provider IO by provider policy and advances later FIFO work without the old worker. Directly covered by `receiver_durable_reconstruction_matrix`; delivery-store restart tests remain focused support. |
| `retrying` | The fresh App resumes the due retry from the frozen envelope, finishes once, and advances later FIFO work. Directly covered by `receiver_durable_reconstruction_matrix`; delivery reopen tests remain focused support. |
| `acknowledged` | The fresh App preserves the exact acknowledgement, performs no provider replay, and advances later FIFO work. Directly covered by `receiver_durable_reconstruction_matrix`; post-ack suites remain focused support. |
| `failed`, `done` | The fresh App never reclaims either terminal row and advances a later queued job. Directly covered by `receiver_durable_reconstruction_matrix`; terminal and acknowledgement suites remain focused support. |
