...
- **Opaque-ingress workspace routing.** `server/router.rs` exhaustively proves
  ...
  owners, fields, and returned values without a same-basename fallback. It
  ...
  aliases, qualified-self UFCS, controller fields and return chains, Unix
  sockets, channels, and queues. Negative fixtures prove unrelated same-name
  methods and ordinary interactive controller APIs remain disconnected.
