...
| `tests/entry_collect.rs` | `entry::collect` against real temp directory trees, including the `capture/` in-basket, walked like any other bucket and **verbatim**, so a user's spaces, capitals, and nesting survive into the match text. |
