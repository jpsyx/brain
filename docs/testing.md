...
- **Durable receiver state.** Focused tests under `state/receiver/tests/`
  ...
  exact native binding; ownerless recovery persistence and reopen discovery;
  recovery exhaustion; absolute expiry; legacy completion ambiguity; pending
  notice intent; FIFO release; and unsafe resume terminalization. Two store
  handles prove only one full-snapshot CAS publishes an effect. Exact completion
  winning first defeats reconciliation, while reconciliation winning first
  fences late progress, completion, renewal, and process-exit writes. Recovery
  claim tests prove claiming only an already-due attempt preserves identity and
  first facts, consumes no additional budget, establishes the launch deadline,
  and retains workspace-wide live-claim exclusion. Completion remains atomic across the
  ...
  finite accepted-work deadlines; partial-v10 repair restores
  immediately reconcilable deadlines, and v10 down terminalizes recovery work
