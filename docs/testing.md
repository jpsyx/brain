...
- **Opaque-ingress workspace routing.** `server/router.rs` exhaustively proves
  ...
  through aliases and generic wrappers plus any TUI receiver `*Lease` type.
  ...
  channels, queues, consuming iteration, endpoint representation, warm-panel
  lease representation, selected-panel access, and blocking waits. Negative fixtures prove a
