...
- **Composed multi-workspace acceptance.**
  ...
  process edges. Claude, Codex, OpenCode, and pi produce selected-root launch specs;
...
- **Filesystem collection** (integration). `entry::collect` against real
  ...
  skipping, root-skipping, tolerance of a missing bucket, plus
  `collect_with(Hidden::Include)` walking into a dotted directory and marking
...
- **Facade source boundary.** `tests/agent_registry_boundary.rs` rejects public
  ...
  normalized observation requests through Claude, Codex, OpenCode, and pi, proving
  ...
  accepted-turn correlation for every frontend's own payload shape (Claude's
  `prompt_id`, Codex's `thread_id`, and the `turn_id` OpenCode and pi report),
  revocation by a later
...
- **Persistent Manual sessions.** `manual_session::tests` covers trimmed,
  ...
  to prove exact manual mapping rotation for Claude, Codex, OpenCode, and pi,
...
- **Durable receiver state.** Focused tests under `state/receiver/tests/`
  ...
  damaged, exhausted, and idempotent rows. Recovery-policy tables cover every
  state, equality expiry, separate launch and accepted-recovery budgets,
  authorization-time deadlines, future-skewed evidence, and saturating
  arithmetic. Store reconciliation tests cover wait at the millisecond below a
  ...
  read and write through the first transaction. Exact completion winning first
  defeats reconciliation, while reconciliation winning first fences late
  progress, completion, renewal, and process-exit writes. Recovery claim tests
  ...
  lifetime/current-attempt evidence split. The v9-to-v10 matrix makes renewed
  claims and future-skewed launches immediately due while deriving only safe
  finite accepted-work deadlines; partial-v10 repair restores immediately
  reconcilable deadlines. Adjacent 0.84.8 startup tests damage each cleanup
  identifier separately, then prove exact registration/session reconstruction,
  notice intent, cleanup redrive after reopen, wrong-ack rejection, exact-ack
  registration and lock release, and cessation of redrive. Ambiguous and
  effective-session evidence plus unique frontend, actor, and channel mismatches
  for both missing halves fail closed without releasing unproved resources.
  Store tests also prove complete-tuple acknowledgement repeats those three
  conversation-attribution checks before releasing a registration or lock,
  while v10
  down removes both cleanup columns and terminalizes recovery work
  before the v9 downgrade matrix crosses every lifecycle state, down, and the
  old v8 claim query, proving all
  ...
  cleanup tuple and a dead recorded PID; a matching live process fails closed.
- **Receiver dispatch state.** Composed `tui::app_brain::tests` drive the one
  ...
  the next tick; Claude, Codex, OpenCode, and pi each receive a new isolated
  ...
  BR-16 recovery tests additionally pin reconcile-before-restart ordering,
  shutdown and exact artifact cleanup before durable acknowledgement, recovery
  priority over later FIFO work, persisted-frontend command selection, exact
  ...
  terminal-notice migration to the generic durable outbox before FIFO advancement, and reconstruction
  ...
  exact cleanup or activation. The spawned-cleanup matrix keeps shutdown failing
  through launch-deadline equality, attempts a competing native-session claim,
  ...
  registry after queue acceptance, then prove Claude, Codex, OpenCode, and pi launch
...
- **The interactive event loop.** `TuiRuntime` opens `/dev/tty`, toggles raw
  ...
  terminal or a live Claude/Codex/OpenCode/pi provider process.
...
multi-drop `/restart` response intents, frozen-authority fallback/no-fallback,
generic concurrent delivery claims, obsolete-API structural checks, and themed
privacy-redacted status counts for every durable phase. Review-fix coverage adds
all-kind missing-row/table and valid-generic repair/down mapping, same-version
...
reconstruction independently for Claude, Codex, OpenCode, and pi through
...
| `tests/entry_collect.rs` | `entry::collect` against real temp directory trees, including the `capture/` in-basket, walked like any other bucket and **verbatim**, so a user's spaces, capitals, and nesting survive into the match text, and the opt-in `collect_with(Hidden::Include)` walk that the tree's hidden-files toggle uses. |
...
| `tests/pi_acceptance.rs` + `tests/fixtures/pi/fake_pi.sh` | The facade driving a real fake pi process: the argv pi receives (declined project trust, Brain's extension by path, Brain's session id, and an option-looking prompt kept as one argument after `--`), the exact terminal bytes for typing, submitting, a busy-turn follow-up, and a new session, the Brain identity environment, and no inherited provider secret. |
| `tests/pi_extension.rs` + `tests/fixtures/pi/extension_harness.js` | The real pi bridge under Bun/Node with a stand-in ExtensionAPI: registered events, `session_start` reason as bridge source, an ephemeral session with no id, last-assistant selection, error and abort suppression, settle-once publication, marker-only receiver authority with a minted turn id bounding its tool events, stdin-only payloads with a minimal environment, a clean no-op without `BRAIN_ROOT`, and one artifact through the real Python bridges and state database. |
| `agent::pi::probe` tests + `tests/fixtures/pi/fake_pi.sh` | A stand-in `pi` answering the three read-only probe questions, so no test depends on the developer's own pi install, configuration, or provider credentials: version floor (below, exactly at, newer, pre-release, and output from some other program), a `--help` listing missing one flag Brain appends, an unreadable model catalog, an empty one, and a healthy install. `agent::pi::sessions` tests cover pi's working-directory encoding, both directory overrides, and exact session-file matching. |
| `tests/startup_migration/pi_frontend.rs` | A pre-pi database gains the widened `agent_kind` contract by being opened, then accepts a pi row; a downgrade restores the previous contract, drops only the pi rows, and rebuilds the one-main index the table rebuild removed. |
| `tests/startup_migration/capture_directory.rs` | Upgrading to 0.94.0 gives **every** registered workspace root a `capture/` in-basket; reconciling the current version restores one that was deleted; a downgrade removes an empty in-basket but leaves a filled one (and its file) untouched, because a rollback must never trade the user's captured material for a tidy listing. |
...
| `workspace::templates` tests | The seeded `AGENTS.md`: written when absent, never overwriting an edited copy, idempotent, describing every top-level directory itself (so it needs no companion document, and links none), carrying nothing instance-specific (no `~/brain`, absolute paths, private skills directory, or personal names), and naming only skills Brain actually bundles — checked in passages that discuss skills, so an example project slug is not mistaken for one. Seeding writes **no** `README.md`, and an existing one is left untouched. |
