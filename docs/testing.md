...
- **Opaque-ingress workspace routing.** `server/router.rs` exhaustively proves
  ...
  Unicode identifiers and lifetimes, where-clause function types,
  const-generic braces, generic defaults, ordinary/raw/byte/C literals, nested
  blocks, associated and foreign items, cfg implication, and macro-generated
  storage. The guard permits local transient job values, use of the opaque
  `InboundQueue` API, and only canonical-path one-shot payload shapes in the
  top-level item at the exact `src/tui/receiver/effect.rs` path. `syn` does not
  perform procedural expansion, and declared types cannot reveal type-erased
  runtime contents; these remain manual branch-review limits. Socket characterization
...
| `state::receiver::tests` + `state::database::configuration_tests` | Durable job/conversation identity, lifecycle, non-destructive FIFO claim bundles, one live workspace claim, exact-owner launch CAS, bounded pre-acceptance retry, conservative progressed recovery, lifecycle-reported binding-only replacement, provider-first deduplication, atomic queued capacity, concurrent final-slot admission, transcript preservation, scope checks, numeric safety, foreign keys, reopen persistence, and receiver-specific pre-migration SQLite lock budgeting. |
...
| `tui::receiver::{session_tests,failure_tests}` | Unique remote instances distinct from the main TUI, exact fresh/resume session ownership, registration-guard cleanup, main-lineage preservation, and controller/session/durable retry rollback for every pre-acceptance failure class. |
| `tests/startup_migration.rs` | Compiled ordinary-startup reconciliation plus explicit downgrade for lifecycle integrations and receiver schemas v6/v7 across every registered workspace that already has a state DB; absent DBs remain absent until first `Db::open`, and help/version remain side-effect free. |
