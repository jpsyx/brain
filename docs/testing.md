...
| Concurrent empty-remote setup elects one owner | `sync::identity::tests::concurrent_empty_setup_elects_one_claim_without_overwriting_the_manifest` uses barriers at empty observation and claim publication, with no sleep, to prove exactly one canonical manifest publication and one winner. |
| Schema transition survives the legacy/current boundary | `tests/multi_workspace_migration/schema_transition.rs` proves CSVs, durable baselines, then schema ordering and retryable failure. `tests/sync_local/schema_transition.rs` uses real rclone for final legacy sync, migration, immediate current sync, and second-machine legacy join/convergence. |
