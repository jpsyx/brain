...
- **Durable receiver state.** Focused tests under `state/receiver/tests/`
  ...
  arithmetic. Store tests prove claim/launch/acceptance deadline establishment,
  a real policy-checked recovery claim that preserves identity and first facts,
  consumes only the recovery budget, establishes bounded launch/recovery
  deadlines, resets to a fresh revision-zero cursor, and leaves every field
  unchanged when its comparison fails. Completion remains atomic across the
