...
| `default_tui_view` | env (machine-global) | Q2: machines may use different startup preferences. Q2a: the preference describes this installation's TUI, so every workspace on one machine shares it. |
...
| `default_tui_view` | machine-global | `tasks` | TUI startup target on **this machine**. Exactly one of `tasks`, `brain_dir`, or `brain_llm`. `tasks` and an unset value focus the tasks view when tasks exist; an empty task store shows the brain-directory view instead. `brain_dir` focuses that view. `brain_llm` focuses the brain panel while leaving tasks on the main-view side, or brain directory there when the task store is empty. |
...
| `skill_sessions` | workspace record | *(unset → daily triage only)* | The **skill sessions** this machine offers in the command palette: a JSON array of `{title, prompt, command_label}`. Each runs its prompt in its own brain-panel tab and closes when the run signals completion; while it runs, its palette row disappears. `prompt` is required, `title` defaults to it, `command_label` defaults to `Run <title>`. Daily triage is **builtin** (offered while `enable_daily_triage_check` is on) and is neither listed nor removable here. Machine-local because a definition names a skill that must actually be installed on *this* machine. `brain env set skill_sessions` with no value opens an add/edit/delete walkthrough. See [features.md](features.md) and [data-model.md](data-model.md). |
...
**Automatic machine migrations.** Every Brain command except help and version
first migrates machine-owned state to the compiled version and reconciles the
managed target state. This includes commands whose later workspace bootstrap is
read-only. The stamp is
`$XDG_CONFIG_HOME/brain/migrations/version` (fallback
`~/.config/brain/migrations/version`). `install.sh` performs the same transition
when replacing an existing binary, using an up migration for upgrades and a
down migration for downgrades. Current migrations are idempotent: they remove
superseded state, preserve unrelated user configuration, and recreate missing
managed artifacts.

**Workspace-registry migration.** When bootstrap finds an `env.json` that is invalid or not at the
