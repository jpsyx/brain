...
| **receiver-run tab** | A distinct ephemeral brain-panel tab reserved for one durable receiver job and its own controller. Insertion is background-only: it never selects the tab, reveals the panel, changes the main view, or moves keyboard focus. The shared storage seam exists now; the later BR-14 durable coordinator owns creating production runs. | metadata and controller in `tui::state::brain::ephemeral`; background access through `tui::state::BrainPanelState` |
