...
| **capture** / **the in-basket** | The user-managed `capture/` directory in a workspace root, where they dump notes, photos, and files before anything decides where those belong. Brain only guarantees it exists and keeps it searchable; filing its contents is the `second-brain` skill's job, and `triage weekly` runs that pass. It is never a filing destination. | `workspace::ensure_capture_directory` / `CAPTURE_DIRECTORY` (`src/workspace/initialize.rs`); `Bucket::Capture`; `startup_migration::capture_directory`; `skills/second-brain/SKILL.md` |
