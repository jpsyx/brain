...
| **sub-view** | A mode *inside* one main view, never a fourth entry in `MainView::CYCLE`. Two main views have them. The **tasks view**'s sub-views are its tabbed modes (`today`, `mit`, `past_due`, `week`, `habits`, `backlog`, `all`); these were called "views" in the old `tasks` project, and `Tab` / `Shift+Tab` cycle them. The **brain-directory view**'s sub-views are the fuzzy **search** picker and the directory **tree**; `Alt+Enter` switches to the tree on the highlighted entry, and `Alt+Enter` or `Esc` returns. | tasks: `view::View` + `View::CYCLE` (`src/tasks/view/`); brain directory: `tui::state::BrainDirView` (`src/tui/state/shell.rs`) + `src/tree/` |
| **tree sub-view** / **the tree** | The brain-directory view's directory tree: the entries the search picker already collected, nested under the current search scope, with a synthetic `../` row that re-roots one level up and never climbs above the brain root. | `tree::TreeView` (`src/tree/`); glue in `src/tui/tree_view.rs`; `EntryCommand::Explore` |
