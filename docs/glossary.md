...
| **brain panel** | The live agent chat session in a PTY. Claude is the default; `--codex` / `-cx` selects Codex. `--open-code` / `-oc` is reserved and fails before the shell because OpenCode is not functional yet. App-level: the panel does **not** belong to a main view and stays open across a view switch. (Formerly called the *claude panel* in the `tasks` project.) | `src/agent/controller.rs` (`AgentController`); `src/pty_pane.rs` (`PtyPane` transport); `App.brain: Option<AgentController>`; `App.agent_kind` |
