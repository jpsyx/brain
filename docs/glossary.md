...
| **session store** / **state DB** | The workspace-UUID-scoped SQLite DB (`~/.cache/brain/workspaces/<workspace-uuid>/state.db`) that tracks every registered frontend by frontend, workspace, actor, and channel (lock + recency). Claude validates transcripts, OpenCode validates live root sessions from the selected workspace, and Codex validates that its own rollout file for the id is still on disk. | `WorkspacePaths::state_db`; `src/state.rs`; `src/agent/opencode/session.rs` |
