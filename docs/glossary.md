...
| **receiver-run tab** | A distinct ephemeral brain-panel tab reserved for one claimed durable receiver job, unique remote instance, and dedicated `AgentController`. The event-loop coordinator inserts it in the background, so launch and terminal close never select a tab, reveal the panel, change the main view, or move keyboard focus. | coordinator in `tui::app_brain::receiver`; metadata and controller in `tui::state::brain::ephemeral` |
