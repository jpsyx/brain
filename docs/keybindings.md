...
| `Alt+[` / `Alt+]` | Cycle the brain-panel tab (previous / next): **main** session ↔ each open **skill session** | The reliable switch (resolves as Alt-modified brackets or the macOS Option smart-quote glyphs). No-op unless a skill session is open. The command palette (`Ctrl+P`) also carries **Show main brain session** and one **Show \<title\> session** row per open tab. `Alt+1` selects the main session and `Alt+<n>` the nth skill session directly on terminals that support Alt+digit, but that encoding is unreliable, hence the bracket cycle. On macOS layouts that send the Option-produced glyph instead (`¡`, `™`, `£`, …) those glyphs address the same slots; addressing a slot with no tab behind it selects nothing, and because such a glyph is also ordinary text it still reaches the panel. From either panel |
| `Ctrl+X` | Close the brain panel (ends its agent session) | The main view goes full-width. From either panel. **On a skill-session tab it closes only that ephemeral session**, leaving the main session up |
