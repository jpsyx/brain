...
| Durable receiver state | `state/receiver/` | `identity.rs` owns logical conversation identity; `job_state.rs` owns lifecycle transitions; `recovery_policy.rs` owns the clock-injected pure lease decision; `schema.rs` owns atomic schema v10, conservative repair, and v10/v9 downgrade mapping, while `schema/token.rs` owns semantic UUID token reconciliation; `store.rs` owns acceptance and conversation mutations; `store/load.rs` owns typed row decoding; `store/observation.rs` owns exact token, owner, instance, live-session, revision, and lifecycle-deadline commits; `store/completion.rs` atomically keeps first lifecycle facts separate from the current-attempt cursor while binding the exact completed session and terminal job state; `store/reconciliation.rs` owns the immediate oldest-blocker transaction and neutral semantic effects, with focused candidate and terminal helpers beneath it; `store/claim/live.rs` owns the workspace-wide live-claim exclusion shared by every claim transaction; `store/claim/next.rs` owns ordinary FIFO selection and refuses due recovery attempts; `store/claim/recovery.rs` discovers and claims only an ownerless recovery already persisted by reconciliation, without spending its budget again; `store/claim.rs` owns launch CAS, transitions, and bounded failures that remain provably pre-spawn; `store/session.rs` owns exact receiver registration, release, and lifecycle binding attribution |
...
ownerless due recovery with the exact native binding preserved; exhaustion,
absolute expiry, missing resume evidence, and incomplete legacy completion
become terminal with a pending unavailable-notice intent. Reopen discovery may
claim only that persisted recovery. The transaction returns content-free effect
identifiers, while controller cleanup, validation, launch, and notice delivery
remain outside the state layer.
...
observation split, and the pending unavailable-notice intent.
See
