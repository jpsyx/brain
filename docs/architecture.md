...
| Durable receiver state | `state/receiver/` | `identity.rs` owns logical conversation identity; `job_state.rs` owns lifecycle transitions; `recovery_policy.rs` owns the clock-injected pure lease decision; `schema.rs` owns the receiver schema coordinator, `schema/recovery.rs` owns schema-v10 columns, repair, and v10-to-v9 downgrade mapping, `schema/recovery/cleanup.rs` reconstructs cleanup authority only from one exact registration/session match, and `schema/token.rs` owns semantic UUID token reconciliation; `model/delivery/decode.rs` owns serialized-envelope validation; `store.rs` owns acceptance and conversation mutations; `store/load.rs` owns typed row decoding; `store/observation.rs` owns exact token, owner, instance, live-session, revision, and lifecycle-deadline commits; `store/completion/{authorization,duplicate,lifecycle,transaction}.rs` respectively own immutable completion proof, replay validation, lifecycle merging, and the atomic transcript, outbox, binding, answer-ready, claim-release, and cleanup-authority write; `store/delivery/{claim,decode,result,reconciliation}.rs` respectively own exact claim and pre-provider release, typed row decoding, result CAS, and expired-lease recovery behind the delivery facade; `store/answer_cleanup.rs` owns durable controller-shutdown fencing, exact session release, and durable artifact acknowledgement after agent ownership ends; `database.rs` owns atomic dead-lock reap plus handoff acknowledgement; `store/reconciliation.rs` owns the immediate oldest-blocker transaction and neutral semantic effects, with focused candidate, cleanup-acknowledgement, and terminal helpers beneath it; `store/claim/live.rs` owns the workspace-wide live-claim exclusion shared by every claim transaction; `store/claim/next.rs` owns ordinary FIFO selection and refuses due recovery attempts; `store/claim/recovery.rs` gates recovery on the globally oldest claimable or blocking row, then claims only an ownerless, cleanup-acknowledged recovery already persisted by reconciliation; `store/claim.rs` owns launch CAS, transitions, and bounded ordinary failures that remain provably pre-spawn; `store/session.rs` owns exact receiver registration, release, and lifecycle binding attribution |
...
instance/session cleanup fence. The App shuts down that superseded native run,
removes its exact artifacts, and acknowledges the exact fence before a recovery
claim can proceed. Both
...
later upgrade can finish exact cleanup. The 0.84.12 receiver-notice migration
...
controller-shutdown acknowledgement fields before rebuilding indexes, verify
managed index uniqueness and columns, and fail closed when
...
the next effect can begin. Ordinary continued-message resume validation
false/error and exact resume registration rejection choose a portable Fresh
plan only after that renewal;
...
child-exit, and cleanup management. A terminal
...
abandon a pending or active claim: the same tick continues renewal and
freshness for a pending claim and completion, child-exit, and cleanup management
for an active run. A pending claim cannot start a new ordinary or recovery
process until re-enable;
