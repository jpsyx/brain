...
- [x] Consistency: render the **non-JSON** `brain sync conflicts` list through
  ...
  Implemented on `fix/conflicts-list` in `2ac39a0`. Validation:
  `cargo test --release` green (653 tests; watcher timing test still ignored by
  design) and `cargo clippy --release --all-targets` clean.
...
- [x] `rclone crypt` (zero-knowledge client-side encryption) — a clean seam was
  ...
  is the user's responsibility. Landed in `22c1ddb`
